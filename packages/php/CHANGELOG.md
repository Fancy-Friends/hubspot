...
- **`fancy-flow-php` executors for every node.** `src/Flow/` carries one `#[FlowNode]` class per action and trigger, and `HubspotFlow::EXECUTORS` lists them.

A Laravel host running fancy-flow-php could show this connector's nodes in its editor and could not run them: `particle-academy/hubspot-php` shipped the request builders and no executor. Each one is the PHP twin of the executor in `@particle-academy/hubspot-js` — the same kind, the same request, the same value on `out` — and an unsafe-to-replay action derives its idempotency key from the run and the node, so a retried durable run sends the key it sent the first time.

Register them by adding `vendor/particle-academy/hubspot-php/packages/php/src/Flow` to `config('fancy-flow.discover')`. `particle-academy/fancy-flow-php` is SUGGESTED, not required, and conflicts outside `>=0.51.0 <2.0`, the range the executors were tested under. Nothing outside `Flow\` needs it.

### Fixed

- **Fake mode through `ConnectorClient` threw.** `Hubspot::descriptor()` handed the connector core its faker as `HubspotFaker::respond(...)`, which takes `($operation, $request)`; the core calls a faker `($operation, $config, $fake, $input)`. So `$config` arrived as `$request` and every fake call died on "Call to a member function … on null". The descriptor now translates between the two. Calling `HubspotFaker::respond()` directly — what this package's own tests do, which is why they never saw it — is unchanged.
...
`npm view @particle-academy/hubspot-ui@0.3.2` shows the attestation; releases before
this one have none.
...
`@particle-academy/hubspot-ui` and `@particle-academy/hubspot-js` live at
...
- **`@particle-academy/hubspot-js` now accepts a RANGE of `@particle-academy/hubspot-ui`, not one exact version.**

It peer-depended on `@particle-academy/hubspot-ui` at exactly the release it shipped with. That is the
...
What it cost: ship `@particle-academy/hubspot-ui` with a fixed help string and every consumer on the
previous `@particle-academy/hubspot-js` had an **unmet peer**, which npm 7+ errors on. A documentation
...
Nothing else changed. `particle-academy/hubspot-php` and `fancy-hubspot` are unaffected; neither has an
equivalent edge.
...
[0.3.1]: https://github.com/Fancy-Friends/hubspot/releases/tag/v0.3.1
