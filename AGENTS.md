...
npm run provider -- hubspot            # regenerate this repo
npm run provider -- hubspot --check    # exit 1 on any difference
```
...
| `packages/ui` | `@particle-academy/hubspot-ui` — the authoring surface, React, every host | Generated |
| `packages/js` | `@particle-academy/hubspot-js` — Node, on `@particle-academy/fancy-connector-core` | Generated |
| `packages/php` | `particle-academy/hubspot-php` — PHP 8.4, on `particle-academy/fancy-connector-core` | Generated |
| `packages/py` | `fancy-hubspot` — Python 3.11+, stdlib only | Generated |

The authoritative copy of `provider/` lives in the weaver envelope at
`providers/hubspot/provider/`. The copy here is emitted from it and `--check`
...
| | Namespace |
|---|---|
| This repo | `github.com/Fancy-Friends/hubspot` |
| npm | `@particle-academy/hubspot-ui`, `@particle-academy/hubspot-js` |
| Packagist | `particle-academy/hubspot-php` |
| PyPI | `fancy-hubspot` |
...
**Default to plain HTTP.** A HubSpot SDK is third-party code and is subject to
